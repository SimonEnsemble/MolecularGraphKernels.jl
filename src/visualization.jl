...
viz_node_labels(::Type{T}, graph::MetaGraph) where {T<:MetaGraph} =
    ["$v" for v in vertices(graph)]
viz_node_labels(::Type{T}, graph::ProductGraph) where {T<:ProductGraph} =
    ["$(get_prop(graph, v, :v₁v₂_pair))" for v in vertices(graph)]
...
function viz_graph(
    ...
    plot = gplot(
        ...
        nodefillc = [
            RGB((rc[:cpk_colors][atomsymbol(atom)] ./ 255)...) for

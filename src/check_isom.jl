...
is_isomorphic(A::SimpleGraph, B::SimpleGraph) =
    is_isomorphic(A, B; edge_labels = Symbol[], node_labels = Symbol[])
is_isomorphic(A::AbstractMatrix, B::AbstractMatrix) =
    is_isomorphic(SimpleGraph(A), SimpleGraph(B))
is_isomorphic(A::AbstractMatrix, B::ProductGraph) = is_isomorphic(A, adjacency_matrix(B))
is_isomorphic(A::ProductGraph, B::Union{ProductGraph,AbstractMatrix}) =
    is_isomorphic(adjacency_matrix(A), B)

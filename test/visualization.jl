...
function test_vis(graph::AbstractMetaGraph, graph_name, set_name; style=:spring, edge_mask=ones(ne(graph)), node_mask=ones(nv(graph)))
    @testset "$set_name" begin
        ...
        @test !isnothing(viz_graph(graph; savename=graph_name, layout_style=style, edge_alpha_mask=edge_mask, node_alpha_mask=node_mask))
...
test_vis(mol::GraphMol, graph_name, set_name; kwargs...) = test_vis(MetaGraph(mol), graph_name, set_name; kwargs...)

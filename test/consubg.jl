...
@testset "ConSubG" begin
    ...
    @test any([
        i in ⊗ₜ([[2, 6, 7]], [[]], T_from_algorithm) for
        i in collect(permutations([2, 6, 7]))
    ])
    ...
    @test all(
        i in sort.(⊗ₜ([[2, 3], [2, 5]], [[8]], T_from_algorithm)) for
        i in sort.([[2, 3, 8], [2, 5, 8]])
    ) && all(
        i in sort.([[2, 3, 8], [2, 5, 8]]) for
        i in sort.(⊗ₜ([[2, 3], [2, 5]], [[8]], T_from_algorithm))
    )

    @test all(
        i in sort.(ConSubG(4, G)) for
        i in sort.([[4, 2, 1, 5], [2, 3, 1, 5], [3, 4, 1, 5], [3, 2, 1, 4]])
    ) && all(
        i in sort.([[4, 2, 1, 5], [2, 3, 1, 5], [3, 4, 1, 5], [3, 2, 1, 4]]) for
        i in sort.(ConSubG(4, G))
    )

    @test all(
        i in sort.(⊗ₜ([[6, 7]], [[8]], T_from_algorithm)) for i in sort.([[6, 7, 8]])
    ) && all(i in sort.([[6, 7, 8]]) for i in sort.(⊗ₜ([[6, 7]], [[8]], T_from_algorithm)))

    @test all(
        i in sort.(CombinationsFromTree(T_from_algorithm, 4, 1)) for
        i in sort.([[4, 3, 2, 1], [3, 2, 8, 1], [5, 2, 8, 1], [7, 6, 8, 1]])
    ) && all(
        i in sort.([[4, 3, 2, 1], [3, 2, 8, 1], [5, 2, 8, 1], [7, 6, 8, 1]]) for
        i in sort.(CombinationsFromTree(T_from_algorithm, 4, 1))
    )

    @test all(
        i in sort.(CombinationsWithV(1, 4, G)) for
        i in sort.([[4, 2, 1, 5], [2, 3, 1, 5], [3, 4, 1, 5], [3, 2, 1, 4]])
    ) && all(
        i in sort.([[4, 2, 1, 5], [2, 3, 1, 5], [3, 4, 1, 5], [3, 2, 1, 4]]) for
        i in sort.(CombinationsWithV(1, 4, G))
    )
end
end

...
function all_methods(mainmod::Module=Main)
    ...
    index(x) = index_func(x)

    function index(x::Function)
        index_func(x)
        return index_func(typeof(x))  # all functions have a type
    end
...
function hunt(
    methods::Union{Base.MethodList, AbstractVector{Method}},
    mod::Module;
    exclude::Vector{<:Type}=[]
)
    function is_pirate(meth::Method)
        ...
        try
            ...
            (
                ...
                function_pkg == Base.PkgId(Core) && method_pkg == Base.PkgId(Base)
            ) && return false
        catch err
